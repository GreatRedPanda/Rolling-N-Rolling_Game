...
namespace BezierSolution
{
	...
	public class BezierSpline : MonoBehaviour
	{
        ...
        float _ratio;
        public void InitializeArcs( int resolution)
        {
            _resolution = resolution;
            _ratio = 1f / resolution;
            _arcs = new float[resolution];
            _timeVals= new float[resolution];
            _arcsPoints = new Vector3[resolution];
            float cLen = 0;
            Vector3 prevPoint = GetPoint(0);
            _arcsPoints[0] = prevPoint;
            _arcs[0] = 0;
            for (int i = 1; i < _arcs.Length; i++)
            {
                _timeVals[i] = (float)i *_ratio;
                Vector3 nextP = GetPoint(_timeVals[i]);
        ...
        public Vector3 GetArcParametrizedTime(float u, ref float normalizedTime)
        {
            if (u <= 0) return GetPoint(0);
            if (u >= 1) return GetPoint(1);

            int index = 0;
            int low = 0;
            int high = _resolution - 1;
            ...
            if (index < 0) return GetPoint(0);
            if (index >= _resolution - 1) return GetPoint(1);

            // Linear interpolation for index
            float min = _arcs[index];
            float max = _arcs[index + 1];
        
            float interp = (target - min) / (max - min);
             normalizedTime = (index + interp + 1) * _ratio;
            return GetPoint(normalizedTime);


        }


        public float GetPointlengthPercent(Vector3 point)
        {
            float percent = 0;
            ...
            float distBetweenArcsPoints = 0;
            for (int i = 1; i < _arcsPoints.Length; i++)
            {

                float dist = Vector3.Distance(point, _arcsPoints[i-1]);

                float dist2 = Vector3.Distance(_arcsPoints[i - 1], _arcsPoints[i]);
              
                if (dist < dist2)
                {
                    distBetweenArcsPoints = dist;
                    arc = i-1;
                    break;
            ...
            lengthPoint = _arcs[arc] + distBetweenArcsPoints;
            percent = lengthPoint / totalLength;
            return percent;
